...
# Function will update credentials on active context using API.  Bypasses need for vsphere kubectl plugin and only updates active kubeconfig file.
# Requires KUBECONFIG set to -supervisor.yml with config already present.

# UPDATE THIS VARIABLE
_KUBECONFIG_SUPERVIOR_USERNAME="first.last"

ksphere() {
    if [[ "$KUBECONFIG" =~ \-supervisor.yml$ ]]; then 
        kubectl config set-credentials $_KUBECONFIG_SUPERVIOR_USERNAME --token=$(curl -s -H 'User-Agent: kube-plugin-vsphere bld 20919387 - cln 10649850' --user $_KUBECONFIG_SUPERVIOR_USERNAME -k --basic -X POST "https:$(kubectl config view | yq .clusters[0].cluster.server | cut -d: -f2)/wcp/login" | jq .session_id -r)
    fi
}
...
decode_k8s_secret() {
  if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: decode_secret <secret_name> <namespace> [output_file]"
    return 1
  fi
  ...
  secret_data=$(kubectl get secret "$1" -n "$2" -o jsonpath="{.data}" 2>/dev/null)
  if [ $? -ne 0 ]; then
    echo "Failed to get secret '$1' in namespace '$2'. Please check the secret name and namespace."
    return 1
  fi

  # Decode the secret data
  decoded_data=$(echo "$secret_data" | jq -r 'to_entries | map({(.key): (.value | @base64d)}) | add')

  if [ -n "$3" ]; then
    echo "$decoded_data" > "$3"
    echo "Decoded data written to $3"
  else
    # Pretty print the decoded data
    echo "$decoded_data" | jq -r 'to_entries | map("\(.key): \(.value)") | .[]'
  fi
}